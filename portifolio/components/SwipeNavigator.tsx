...
import { PanResponder, Animated, StyleSheet, View, Dimensions } from 'react-native';
import { useRouter, useSegments } from 'expo-router';

const { width } = Dimensions.get('window');

const TELAS = ['index', 'sobre', 'academico', 'profissional', 'projetos', 'jogo'];
  ...
  const panResponder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponder: (_, { dx, dy }) =>
        Math.abs(dx) > Math.abs(dy) * 1.8 && Math.abs(dx) > 12,
      ...
      onPanResponderRelease: (_, { dx, vx }) => {
        ...
        if (swipedLeft && indiceAtual < TELAS.length - 1 && !swipeHandled.current) {
          swipeHandled.current = true;
          Animated.timing(translateX, { toValue: -20, duration: 100, useNativeDriver: true }).start(() => {
            translateX.setValue(0);
            router.push(`/(tabs)/${TELAS[indiceAtual + 1]}` as any);
          });
        } else if (swipedRight && indiceAtual > 0 && !swipeHandled.current) {
          swipeHandled.current = true;
          Animated.timing(translateX, { toValue: 20, duration: 100, useNativeDriver: true }).start(() => {
            translateX.setValue(0);
            router.push(`/(tabs)/${TELAS[indiceAtual - 1]}` as any);
